...
events.prototype._openDoor_check = function (id, x, y, needKey) {
    ...
    if (needKey && id.endsWith("Door")) {
        ...
        if (!core.hasItem(key)) {
            if (key != "specialKey" && key != "steelKey")
...
events.prototype._changeFloor_afterChange = function (info, callback) {
    ...
    core.status.replay.animate = false;
    core.events.afterChangeFloor(info.floorId, info.fromLoad, info);
...
////// 转换楼层结束的事件 //////
events.prototype.afterChangeFloor = function (floorId, fromLoad, info) {
    if (main.mode != 'play') return;
    return this.eventdata.afterChangeFloor(floorId, fromLoad, info);
}
...
events.prototype.closeDoor = function (x, y, id, callback) {
    ...
    if (!(id.endsWith("Door") || id.endsWith("Wall"))
        || core.material.icons.animates[id] == null || (core.getBlock(x, y) != null && core.getBlock(x, y).block.id != 0)) {
        if (callback) callback();
        return;
    }
    // 关门动画
