...
maps.prototype._canMoveHero_checkPoint = function (x, y, direction, floorId, extraData) {
    ...
    if (nx < 0 || ny < 0 || nx >= core.floors[floorId].width || ny >= core.floors[floorId].height)
        return false;
    if (!core.isReplaying() && (nx < 1 || ny < 1 || nx >= core.floors[floorId].width - 1 || ny >= core.floors[floorId].height - 1))
        return false;
...
maps.prototype._drawThumbnail_drawToTarget = function (floorId, toDraw) {
    ...
    if (toDraw.all) {
        ...
        if (tempWidth <= tempHeight) {
            ...
            ctx.drawImage(tempCanvas.canvas, 32, 32, tempWidth - 64, tempHeight - 64, x + side, y, realWidth, realHeight);
        }
        else {
            ...
            ctx.drawImage(tempCanvas.canvas, 32, 32, tempWidth - 64, tempHeight - 64, x, y + side, realWidth, realHeight);
        }
    }
    else {
        // 只绘制可见窗口
        var offsetX = 1, // core.clamp(centerX - core.__HALF_SIZE__, 0, width - core.__SIZE__),
            offsetY = 1; // core.clamp(centerY - core.__HALF_SIZE__, 0, height - core.__SIZE__);
