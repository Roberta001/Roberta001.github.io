...
ui.prototype.drawWindowSkin = function(background, ctx, x, y, w, h, direction, px, py) {
    ...
    // 仿RM窗口皮肤 ↑
    var c = parseInt(w/2);
    dstImage.drawImage(background, 160,90,     16,      6,  x+c-8,      y,     16,      6);
}
...
ui.prototype.drawPagination = function (page, totalPage, y) {
    ...
    if (totalPage < 0) {
        ...
        core.setTextAlign('ui', 'center');
        core.fillText('ui', page, this.HPIXEL, y*32+19);
...
ui.prototype._drawSLPanel_drawRecord = function(title, data, x, y, size, cho, highLight){
    ...
    if (data && data.floorId) {
        ...
        var v2 = "/"+core.formatBigNumber(data.hero.money,true);
...
ui.prototype.statusBar = (function () {
    function statusBar() {
        this.itemMx = [
            ["book", "wand", "fly"],
    ...
    statusBar.prototype.init = function () {
        ...
        bgctx.drawImage(bg, 0, 0, 640, 400);
        bgctx.fillStyle = "#676767";
        bgctx.fillRect(0, 400, 640, 22);
        core.setTextAlign('outerUI', 'center');

        this.toolbarAction = [
            [main.core.openKeyBoard,  main.core.openQuickShop, function() {core.insertAction([{ type: "insert", name: "回收钥匙商店" }]);}],
    ...
    // 更新属性
    statusBar.prototype._update_props = function () {
        core.clearMap("outerUI", 10, 40, 105, 130);
        var tc = "#E1E1E1";
        core.setFont("outerUI", 'bold 16px Verdana');
        // 设置楼层名
        if (core.status.floorId) {
            core.fillText("outerUI", core.status.maps[core.status.floorId].title, 62, 61, tc);
        }
        ...
        statusList.forEach(function (item) {
            ...
            // 大数据格式化
            core.fillText("outerUI", core.getRealStatus(item), 110, curh, tc);
            curh += 24;
        });
        core.setTextAlign('outerUI', 'center');
    }
    ...
    statusBar.prototype._update_equips = function () {
        core.clearMap("outerUI", 525, 40, 105, 95);
        core.setFont("outerUI", 'bold 16px Verdana');
        ...
        drawEquip(525, 48, core.getFlag("nowWeapon"), "#FFCFAE", "无武器");
        drawEquip(525, 96, core.getFlag("nowShield"), "#D1CEFF", "无防具");
    }

    statusBar.prototype._update_keys = function () {
        core.clearMap("outerUI", 525, 140, 105, 75);
        var todraw = [], keyList = ['yellowKey', 'blueKey', 'redKey'];
        keyList.forEach(function(key, i) {
            todraw[i] = core.itemCount(key);
        });
        var baseX = 528, baseY = 142, dn = 2, dc = 0;
        while(dn >= 0 &&  dc < 28) {
            if (todraw[dn]) {
                this.drawKey(keyList[dn], baseX + (dc % 7) * 14, baseY + parseInt(dc / 7) * 17);
                todraw[dn]--, dc++;
            } else dn--;
        }
    }
    ...
    statusBar.prototype._update_infoWindow = function () {
        core.clearMap("outerUI", 525, 220, 105, 100);
    ...
    statusBar.prototype.showInfo = function (itemId) {
        ...
        core.fillText("outerUI", core.material.items[itemId].name, 525 + 32, 220 + 25, "#D1CEFF");
        core.drawImage('outerUI', core.material.images.items, 0, 32 * icon, 32, 32, 525 + 64, 224, 32, 32);
        core.ui.drawTextContent("outerUI", core.material.items[itemId].text, {
            left: 526, top: 256, maxWidth: 105, color: "#D1CEFF"
        });
        this.selectedItem = itemId;
    }

    statusBar.prototype._update_toolBox = function () {
        core.clearMap("outerUI", 520, 310, 115, 80);
        var tools = [];
		if (core.isReplaying()) {
            tools = [
        ...
        for (var i = 0; i < tools.length; i++) {
            for (var j = 0; j < tools[i].length; j++) {
                core.drawImage("outerUI", core.statusBar.icons[tools[i][j]], 0, 0, 32, 32, 525 + j * 34, 318 + i * 34, 32, 32);
            }
        }
    }
    
    statusBar.prototype.isInGameDraw = function (loc) {
        var x = loc.x / core.domStyle.scale, y = loc.y / core.domStyle.scale;
		return x >= 144 && x <= 496 && y >= 24 && y <= 376;
    }

    statusBar.prototype.fixEventLoc = function (loc) {
        loc = core.clone(loc);
        loc.x -= 144 * core.domStyle.scale, loc.y -= 24 * core.domStyle.scale;
        return loc;
    }

    statusBar.prototype.onclick = function (x, y) {
        if (x <= 112 && x >= 13 && y <= 350 && y >= 190) {
            if (core.isReplaying() || core.status.lockControl || core.isMoving()) return;
            var itemId = this.itemMx[parseInt((y - 190) / 32)][parseInt((x - 13) / 33)];
            if (!core.hasItem(itemId)) return;
            if (core.material.items[itemId].cls == "constants") {
                switch(itemId) {
                    case "book": core.openBook(true); break;
                    case "fly": core.useFly(true); break;
                    case "wand": core.insertAction({
                        type: "useItem", id: itemId
                    }); break;
                    case "snow": core.useItem("snow"); break;
                    default: this.showInfo(itemId);
                }
            } else if (itemId != this.selectedItem) {
                this.showInfo(itemId);
            } else {
                switch(itemId) {
                    case "centerFly": core.ui.drawCenterFly(); break;
                    default: core.useItem(itemId);
                }
            }
            return;
        };
        if (x <= 625 && x >= 525 && y <= 382 && y >= 317) {
            var row = parseInt((x - 525) / 32), col = parseInt((y - 317) / 32);
            if (core.isReplaying()) {
    ...
    statusBar.prototype.print = function (text, cnt) {
        if (!text) {
            var _this = this;
            setTimeout(function() {
                if (!_this.infocnt) core.clearMap("outerUI", 0, 400, 640, 22);
            }, 200);
            this.infocnt = 0;
            return;
        }
        core.clearMap("outerUI", 0, 400, 640, 22);
        core.setFont("outerUI", 'bold 16px Verdana');
        core.setTextAlign('outerUI', 'left');
        core.fillText("outerUI", text, 10, 416, "#E1E1E1");
        core.setTextAlign('outerUI', 'center');
        this.infocnt = cnt || 1;
    }
