var functions_d6ad677b_427a_4623_b50f_a445a3b0ef8a = 
{
	...
	if (core.status.maps[floorId].bgm) {
		...
		bgm = core.getFlag("ScenarioBgm", bgm);
		core.playBgm(bgm);
	}
	// 更改画面色调
	...
	// ...可以新增一些其他内容，比如创建个画布在右上角显示什么内容等等
        ...
        "afterChangeFloor": function (floorId, fromLoad, info) {
	...
	if (fromLoad) {
		...
		if (info._floorId == ":next") core.ui.statusBar.print("登上了楼梯");
		else if (info._floorId == ":before") core.ui.statusBar.print("走下了楼梯");
	...
	core.setFlag('talking', 0); //用楼传之后就不能穿墙了
	...
	var guards = []; // 支援
	if (x != null && y != null) {
		if (enemyId == 'octopus' || enemyId == 'magicDragon') {
			core.removeBlock(x - 1, y - 2);
			core.removeBlock(x - 1, y - 1);
			core.removeBlock(x - 1, y);
			core.removeBlock(x, y - 2);
			core.removeBlock(x, y - 1);
	...
	var hint = enemy.name + " 被打败了 ";
	if (core.flags.enableMoney) hint += "，你获得了 " + money + " 金币";
	...
	core.setFlag('talking', 0); //战斗之后就不能穿墙了
	...
	*/
	...
	if (core.getFlag('bomb') > 0) {
		core.status.hero.money += core.getFlag('bomb');
	}

	if (core.status.floorId == 'MT44' && core.terrainExists(6, 8, 'specialDoor') // 某个楼层，该机关门存在
		&&
		!core.enemyExists(5, 9) && !core.enemyExists(7, 9)) // 且守门的怪物都不存在
	{
		core.insertAction([ // 插入事件
			{ "type": "openDoor", "loc": [6, 8] }, // 开门
		]);
	}
	if (core.status.floorId == 'MT45' && core.terrainExists(7, 10, 'specialDoor') // 某个楼层，该机关门存在
		&&
		!core.enemyExists(8, 9) && !core.enemyExists(8, 11)) // 且守门的怪物都不存在
	{
		core.insertAction([ // 插入事件
			{ "type": "openDoor", "loc": [7, 10] }, // 开门
		]);
	}
	if (core.status.floorId == 'MT45' && core.terrainExists(4, 10, 'specialDoor') // 某个楼层，该机关门存在
		&&
		!core.enemyExists(5, 9) && !core.enemyExists(5, 11)) // 且守门的怪物都不存在
	{
		core.insertAction([ // 插入事件
			{ "type": "openDoor", "loc": [4, 10] }, // 开门
		]);
	}
	if (core.status.floorId == 'MT49' && core.terrainExists(6, 9, 'specialDoor') // 某个楼层，该机关门存在
		&&
		!core.enemyExists(5, 10) && !core.enemyExists(7, 10)) // 且守门的怪物都不存在
	{
		core.insertAction([ // 插入事件
			{ "type": "openDoor", "loc": [6, 9] }, // 开门
		]);
	}
	if (core.status.floorId == 'MT49' && core.terrainExists(6, 7, 'specialDoor') // 某个楼层，该机关门存在
		&&
		!core.enemyExists(5, 8) && !core.enemyExists(7, 8)) // 且守门的怪物都不存在
	{
		core.insertAction([ // 插入事件
			{
				"type": "if",
				"condition": "(flag:炸弹不开门==1)",
				"true": [

				],
				"false": [
					{ "type": "openDoor", "loc": [6, 7] },
				]
			}, // 开门
		]);
	}
	...
	if (core.hasItem('cross') && [85, 120, 199].indexOf(enemy.atk) >= 0) {
		hero_atk *= 2;
	}
	if (core.hasItem('knife') && enemy.atk == 600) {
		hero_atk *= 2;
	}
	// 技能的处理
	...
	if (core.hasFlag('495')) {
		core.material.enemys.redKing.hp = 800;
		core.material.enemys.redKing.atk = 500;
		core.material.enemys.redKing.def = 100;
	} else {
		core.material.enemys.redKing.hp = 8000;
		core.material.enemys.redKing.atk = 5000;
		core.material.enemys.redKing.def = 1000;
	}
}
    },
    "actions": {
	...
	*/
	...
	switch (core.getFlag('bgm')) {
	case 1:
		core.playBgm('xiaoluoli.mp3');
		break;
	case 2:
		core.playBgm('wan.mp3');
		break;
	case 3:
		core.playBgm('huang.mp3');
		break;
	case 4:
		core.playBgm('xian.mp3');
		break;
	case 5:
		core.playBgm('chuanshuo.mp3');
		break;
	case 6:
		core.playBgm('ed.mp3');
		break;
	case 7:
		core.playBgm('hua.mp3');
		break;
	case 8:
		core.playBgm('migong.mp3');
		break;
	default:
		break;
	}
	...
	for (var loc in blocks) {
		...
		if (enemy && core.hasSpecial(enemy.special, 18) && !core.hasFlag('shield5')) {
	...
	if (ignoreSteps >= 0) {
		...
		core.ui.statusBar.clearInfo();
		core.ui.statusBar.printEnvironmentInfo();
		core.setFlag('talking', 0); //瞬移之后就不能穿墙了
		return true;
	}
	return false;
},
	...
	*/
	...
	var left = 12, top = 36, right = core.__PIXELS__ - 2 * left, bottom = core.__PIXELS__ - 2 * top;
	...
	core.fillText('ui', 'H5版复刻者: 数码宝贝51 艾之葵', text_start, top+240);
	core.fillText('ui', '花絮 鹿间裕贵', text_start+105, top+260);
}
    }
}